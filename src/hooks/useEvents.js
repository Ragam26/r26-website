...
export function useEvents(eventType = 'events') {
    ...
    const getEvents = async () => {
      try {
        ...
        sessionStorage.setItem(
          STORAGE_KEY,
          JSON.stringify({ timestamp: Date.now(), data: allEvents }),
        )
        setData(allEvents)
      } catch (err) {
        setError(err)
  ...
  const processedData = data.map((event) => ({
    ...
    eventMonth: event.date
      ? getMonthName(parseInt(event.date.substring(5, 7)))
      : null,
...
export function useIInks() {
  ...
  useEffect(() => {
    const getWorkshops = async () => {
      try {
        const response = await api.get('/api/i-inks', {
          params: {
            'pagination[pageSize]': 25,
            'pagination[page]': 1,
            populate: '*',
